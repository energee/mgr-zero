...
| `lib/pos.ts`, `lib/commands/pos.ts`, `lib/supabase/integration-tokens.ts` | Square transport, registered POS operations, and the credential boundary. Catalog publication freezes its exact request body, provider idempotency key, source, expected item/variation versions, connection generation, and owned or explicitly adopted identity before provider I/O. Unknown outcomes replay that body; known version conflicts require a new current-object snapshot. The publication lease admits current Admin or Warehouse only for its concrete durable attempt. |
