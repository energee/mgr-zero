...
5. **A command that writes more than one row is one Postgres function.**
   ...
   currently fail-closed) is the one exemption and says so with an
   `// atomic-exempt:` comment. MGR v1
