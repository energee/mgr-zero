...
| Path | Owns |
| --- | --- |
| `supabase/migrations/*.sql` | Schema, RLS policies, triggers, grants, and the transactional command-request ledger. The only source of truth for data rules. Every file is immutable once committed — `supabase db push` applies a version once and never rereads it, so a schema change is a new migration and `supabase/migrations.lock.json` pins the rest (see `.agents/superpowers/specs/2026-08-31-mgr-schema-decisions.md`). Application roles have read-only table access; every mutation enters through an explicitly granted, idempotent `security definer` RPC with `search_path = ''`, database-derived actor/tenant/role checks, and a canonical request hash. Private helpers and implementation functions are not executable by application roles. |
...
| `lib/commands/purchasing.ts` | Vendors, materials, contracts, purchase orders (create/send/receive), material requirements/planning, and material counts. `send_purchase_order` is an attestation (`sent_via` mailto/external, never `direct` until a provider is approved) — transport is a property, never a `po_status` fork. Lead time is an observed, never-stored vendor view (`ordered_on → received_on`); a contract's remaining commitment is four numbers (committed, received, on order, available). |
| `lib/commands/packaging.ts` | Packaging runs: schedule (brand-first, no tank required), start against one exact source occupancy, close (lot + output movements + BOM consumption, first-expiring-first for lot-tracked materials), and repack (breakdown only, admin/warehouse). |
| `lib/commands/delivery.ts` | Delivery routes: `save_route` (header + stops replaced in one call), `depart_route`, `return_route`, `get_delivery_stop`. A departed route is frozen — no stop can change and it can't return with an open stop. |
| `lib/commands/transfers.ts` | Internal bin-to-bin and cross-location `stock_transfers` (create/submit/pick/receive/cancel) — a document distinct from priced customer `orders`; bin-to-bin moves inside one location are structurally barred from becoming deliveries. |
| `lib/commands/landings.ts`, `lib/commands/search.ts`, `lib/commands/settings.ts`, `lib/commands/tenancy.ts` | The Beer/Work/More landing aggregates; cross-entity `search_entities` (parallel per-entity RLS-bound reads, not a definer function — RLS already decides the rows); brewery/team settings (`update_brewery`, `update_staff_role`, `revoke_staff`, always keeps one admin); the pre-tenant `provision_brewery` caller. |
| `lib/commands/qbo.ts` | QuickBooks Online: OAuth connect/disconnect, customer/item mapping, invoice push and payment sync, and issued-token revoke on a failed OAuth completion. |
...
| `lib/time-window.ts`, `lib/volume.ts` | Domain display helpers: a stored `hh:mm` window (may wrap midnight) as the string a brewer reads; a stored barrel figure in the unit a brewer reads (storage and TTB stay bbl), including `saleVolume` — a shipped line's two-decimal bbl removal total, which `formatVolume` would round to a glyph. |
| `lib/mgr/money.ts` | Cents as text: `money` for anything an operator reads (thousands separators, a true minus sign on a credit) and `dollarsInput` for the text of a dollars field. An eslint `no-restricted-syntax` rule rejects a local `(cents / 100).toFixed(2)` everywhere else; `lib/qbo.ts` is exempt because its decimal string is a wire value, not a display. |
| `lib/mgr/labels.ts`, `lib/mgr/tax-treatments.ts`, `lib/mgr/enums.ts` | How stored enum values read (`sentenceCase`, import kinds) and which values exist: the sale-channel tax treatments, and the keg and production option arrays the pickers draw. `enums.ts` imports nothing, so a `"use client"` form or a shared view under `components/mgr/views/` can read an option list without importing a `lib/commands/<area>.ts` module and registering every command in it; those modules re-export from here so the zod schemas and the pickers stay one list (`tests/boundary.test.ts`). |
