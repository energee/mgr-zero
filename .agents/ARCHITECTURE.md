...
| `app/(frames)/screens/frame/[s]/` | One screen record rendered full-viewport outside any shell, so `/docs/screens`, `/docs/screens-explore`, `/docs/integrations` and the guides can embed it in an iframe at a real viewport width. Statically generated from the inventory; no other route. |
