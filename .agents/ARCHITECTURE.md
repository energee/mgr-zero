...
| `app/(design)/` | Dev-only component gallery (`/design`); its layout 404s outside development. Each frame renders through the public `app/(frames)/screens/frame/[s]` route (built statically, no dev-only twin), the same route `/docs/screens` and `/docs/integrations` embed. |
...
| `content/docs/{screens,integrations}.mdx` | Generated design-inventory pages in the same Fumadocs tree (`screens.mdx` renders `components/mgr/screen-index.tsx` over `SCREENS` directly, `integrations.mdx` the venue frames grouped by product) — not hand-written, cannot drift, and deliberately outside `documentation-agent.yml`'s allowlist (`tests/design-docs.test.ts`). |
