...
| `lib/pos.ts`, `lib/commands/pos.ts`, `lib/supabase/integration-tokens.ts` | Square transport, registered POS operations, and the credential boundary. Catalog publication owns one durable Square parent item per brand, premise, and menu group; poured formats are its variation identities, and a location publish sends the complete derived brand change set. Each item attempt freezes its exact request body, provider idempotency key, source variations, expected item/variation versions, credential generation, catalog-sync generation, and owned or explicitly adopted identity before provider I/O. Unknown outcomes replay that body; known version conflicts require a new current-object snapshot. A newer credential or catalog generation terminally supersedes unresolved old work, and definitive missing or malformed owned provider objects reject their attempts. The publication lease admits current Admin or Warehouse only for its concrete durable attempt. |
