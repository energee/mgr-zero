...
- Feature PRs that carry a real design ruling mark it with a `## Durable decisions (for the dreaming log)` heading (seen from the backend-push PRs, #185 onward). The dreaming agent should search merged PR bodies for that heading first — it is a stronger signal than scanning every diff, and PR authors already wrote it for this file.
