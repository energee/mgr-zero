...
- **`components/mgr/screens.tsx` is the source of truth for MGR screens** (2026-09-02; built from UI plan rev 4). `2026-08-31-mgr-wireframes.html` is now fully retired (2026-09-03, PR #69 ported its last 17 Slack/QuickBooks/Square venue frames into `components/mgr/venue.tsx`). Change navigation/flows and the `SCREENS` array together — the record count is not sacred, a real operator job gets a real body, never an annotation chip. Design review happens at `/design`; the same records also publish read-only at `/docs/screens` (MGR frames) and `/docs/integrations` (the venue frames, grouped by product), both embedding the one public `app/(frames)/screens/frame/[s]` route — a generated page, deliberately outside the documentation agent's allowlist (`tests/design-docs.test.ts`). Staff uses Today/Beer/Work/More; the wholesale portal has its own Order/Orders/Invoices/Account shell. Frames draw only user-visible UI — the annotation, gate-copy, and Today-exemplar rules live in the plan preamble and §5, not here. Edit the typed records directly, never regenerate.
