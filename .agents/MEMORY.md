...
- `content/docs/user-guide.html` is the documentation master, served at `/docs/user-guide`, linking audience-separated `staff-guide.html` and `portal-guide.html`. Together they cover every current screen/action in customer language (steps, fields/options/defaults/limits, permissions, connected effects, corrections, empty/error states, and unavailable controls) without mixing staff and portal instructions or exposing internals.
