...
| `create_recipe`, `create_recipe_version` | Mutable parent / immutable version | Recipe parent is a single mutable row. Version + ingredients use one immutable RPC; correction creates a newer version. Online. |
...
| Materials/production | `get_purchase_order`, `get_material_on_hand`, `list_materials`, `list_vendors_and_contracts`, `list_recipes`, `get_recipe`, `get_cellar_map`, `get_brew_day`, `get_packaging_run`, `trace_lot` |
...
| **Recipes** (3) | Create recipes and immutable versions | Create mutable recipe parent; read/scale; new version reviews all ingredients | Version editor and costing |
