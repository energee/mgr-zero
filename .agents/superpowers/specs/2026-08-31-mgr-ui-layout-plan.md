...
| `raise_invoice_question`, `resolve_invoice_question` | Mutable / hop green | Implemented. A customer raises a question on their own invoice; Sales/Admin answers off-system and marks it answered, clearing the Today item. Neither verb changes invoice money or payment state. `list_invoice_questions` reads the queue. |
| `disconnect_qbo`, invoice write-off, QBO push defaults | External/financial / copper | Program 13 designs, not currently registered actions. Keep their controls gated until approved contracts and durable provider fixtures exist. |
...
| **Record movement** (1) | Append one correct FG event | Intent/type choices cover opening balance, depletion, loss, sample, festival removal, destruction, and adjustment; then SKU/location pickers, positive QtyPad, conditional channel/state/direction, canonical signed preview. Opening balance is the one manual way finished goods enter stock (a packaging run posts the rest), and returns stay in shipment context. | Same form; keyboard entry secondary |
...
1. **Contracts and stable shells:** extend registry/transport with risk, confirmation,
   ...
   resolve FG correction identity/report semantics and durable taproom count storage
   before their commands (shipment `invoice_timing` is already persisted);
