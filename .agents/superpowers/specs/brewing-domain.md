...
User-facing volume displays are labeled **Volume**. Values of at least one barrel stay in bbl, common keg fractions use familiar bbl fractions, and smaller exact totals use the largest whole unit available: gallons before fluid ounces. Ounces describe a container, so they are only used below a gallon; a volume with no clean whole-unit conversion stays in bbl, and anything that is not a number at all reads as a dash. Stored values and every TTB report remain in bbl.

Atomic formats own entered volume and choose from the units allowed by that input (`oz`, `gal`, `bbl`, `mL`, or `L`); composed formats derive it. A SKU only associates a brand with a format. Different volume or packaging, including a different BOM, requires a different format rather than a SKU override.
...
Loss on a batch is ultimately anchored to packaging: the source of truth is how much beer actually made it into packages versus how much wort was produced. Each batch's production baseline is the knockout (wort) volume allocated from its brew logs, adjusted for volume blended into or out of the batch — a deliberate product choice that makes this a total brewhouse-to-package accountability number, so normal fermentation shrinkage (trub, yeast, CO2 blow-off, dry-hop absorption) lands inside "loss" rather than being tracked as a separate yield figure. Along the way, staff can attribute portions of that loss as it happens: transferring less than a vessel held prompts a loss entry, packaging fewer units than planned prompts one, and samples, taproom pours, and destroyed beer are recorded as their own removal types. When a batch is completed, the system reconciles the whole identity — production baseline minus frozen packaged movement volume minus everything already attributed — and automatically records any meaningful remainder (at least 0.05 bbl or 0.5% of the baseline, whichever is larger) as one nonphysical generic-loss row. `batches.completion_adjustment_id` is its immutable system origin; display copy may call it “Completion Reconciliation,” but code never parses a note. Admin or Sales may allocate that root in exact partial amounts to samples, direct cellar Taproom removals, or destruction. Every allocation keeps the source and adds an equal positive loss reversal and negative replacement, so the all-class volume remains unchanged and the signed correction belongs to its actual posting period. Direct cellar Taproom volume freezes the system Taproom channel tax treatment and cannot be filed until its external filing-line mapping is separately approved. Reconciliation is refused while any of the batch's packaging runs is still open. Known limitation: keg fills recorded directly against a batch outside a packaging session are not yet counted as packaged volume.
