...
export function PourForm({ brand, pour }: { brand: { id: string; name: string }; pour?: { id: string; name: string; ounces: number } }) {
  ...
  const form = useCommandForm("upsert_format", {
    build: () => ({ id: pour?.id, name: pourSkuName(name, ounces), basis: "poured", brandId: brand.id, ounces: Number(ounces) }),
    reset: () => { setName(pour?.name ?? ""); setOunces(pour ? String(pour.ounces) : ""); },
  });
  return <CommandForm open={form.open} onOpenChange={form.setOpen} title={`${pour ? "Edit SKU" : "New SKU"} · ${brand.name}`} trigger={<Button variant="outline" size="sm">{pour ? "Edit SKU" : "New SKU"}</Button>}>
    <form onSubmit={form.submit} className="flex flex-col gap-4">
      <SkuView
        ...
        footer={<CommandFormFooter><Button type="submit" disabled={form.submitting || !(Number(ounces) > 0 && Number(ounces) < 1000)}>{form.submitting ? "Saving…" : "Save SKU"}</Button></CommandFormFooter>}
      />
    </form>
  </CommandForm>;
}
