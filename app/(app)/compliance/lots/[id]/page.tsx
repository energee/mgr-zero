...
export default async function LotTracePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const inStock = t.movements.reduce((n, m) => n + m.qty, 0);
  return (
    <>
      {E.back("Compliance months", t.lot.code, undefined, "/compliance")}
      {E.row([t.lot.brand, t.movements[0]?.sku].filter(Boolean).join(" · "), `run ${t.run?.run_no ?? "?"} · packaged ${t.lot.packaged_on}${t.lot.best_by ? ` · best by ${t.lot.best_by}` : ""}`, `${inStock} on hand`)}
      ...
      {E.tape(t.movements.map((m) => [`${m.qty > 0 ? "+" : ""}${m.qty} · ${m.type.replaceAll("_", " ")} · ${m.sku} · ${m.location}`, m.created_at.slice(0, 10)]))}
