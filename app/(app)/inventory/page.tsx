...
import type { MovementInput, MovementKind } from "@/lib/composer/state";
...
type InventorySearch = { page?: string; recordMovement?: string; skuId?: string; locationId?: string; binId?: string; lotId?: string; qty?: string; type?: string; saleChannelId?: string; destState?: string; note?: string };
...
export default async function InventoryPage({ searchParams }: { searchParams: Promise<InventorySearch> }) {
  ...
  return (
    <FinishedGoodsView
      ...
      createAction={canMove ? <MovementForm autoOpen={recordMovement === "1"} initial={initial} skus={skus.map((s) => ({ id: s.id, label: skuLabel(s), bblPerUnit: s.format_volume?.bbl_per_unit == null ? null : Number(s.format_volume.bbl_per_unit) }))} locations={locations} bins={bins} channels={channels} /> : null}
