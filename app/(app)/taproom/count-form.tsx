...
export function TaproomCountForm({ breweryId, snapshot, projection, lotLabels, role }: {
  ...
  return <>
    ...
    <form onSubmit={submit} className="flex flex-col gap-4" aria-labelledby="count-heading">
      ...
      {tracked.length > 0 && role === "taproom" && <p className="rounded-lg border border-warning/50 bg-warning/10 p-3 text-sm">Tracked lots use worksheet row numbers here. Ask Warehouse for the matching labeled worksheet before counting; do not guess a lot.</p>}
      ...
      {state.draft.lines.map((line, index) => {
        const worksheet = line.lotId ? tracked.findIndex((trackedLine) => trackedLine.key === line.key) + 1 : 0;
