...
// states, spec and reads/writes caption the frame. 404 outside development.
          ...
          {SCREENS.map((s, i) => s.step === step && (
            <article key={s.name} className="flex flex-col gap-2">
              ...
              <div className="flex gap-6 overflow-x-auto pb-2">
                {WIDTHS.map((w) => (
                  <iframe key={w} src={`/design/frame?s=${i}`} title={`${s.name} at ${w}px`} width={w} height={820} className="shrink-0 rounded-lg border bg-background" />
                ))}
              </div>
