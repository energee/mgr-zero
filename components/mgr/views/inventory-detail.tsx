  ...
  return <>
    ...
    {model.movements.length ? model.movements.map(m => <div key={m.id} id={`movement-${m.id}`}>
      {E.row(`${Number(m.qty) > 0 ? "+" : ""}${m.qty} · ${m.type.replaceAll("_", " ")}`, "", movementAction?.(m), "", undefined, <>
        ...
        <p className="break-all text-xs text-muted-foreground">{new Date(m.created_at).toLocaleString()} · {m.id}{m.note ? ` · ${m.note}` : ""}{m.ref ? ` · source ${m.ref}` : ""}{m.source_movement_id ? ` · source movement ${m.source_movement_id}` : ""}</p>
      </>)}
