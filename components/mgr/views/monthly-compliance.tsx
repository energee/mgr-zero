...
export function MonthlyComplianceView({ model, lossAction, fileAction }: {
  ...
  return <>
    ...
    {model.losses.length === 0 ? E.info("No completion reconciliation losses posted in this period.") : model.losses.map((loss) => <section key={loss.key} className="rounded-xl border p-4">
      ...
      {loss.allocations.length ? <div className="mt-3 grid gap-2">{loss.allocations.map((allocation) => <Fragment key={allocation.key}>{E.row(allocation.title, allocation.detail, allocation.bbl)}</Fragment>)}</div> : null}
    </section>)}
    ...
    {model.removals.map((row) => <Fragment key={row.key}>{E.row(row.title, "", row.bbl)}</Fragment>)}
    {model.cellarRemovals.length ? <>{E.info("Cellar removals breakdown is explanatory and is already included once in the filing removal totals above. Do not add it again.")}{model.cellarRemovals.map((row) => <Fragment key={row.key}>{E.row(row.title, "non-additive breakdown", row.bbl)}</Fragment>)}</> : null}
