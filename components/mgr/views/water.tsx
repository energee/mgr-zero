...
import type { WaterAddition, WaterDraft } from "@/lib/mgr/recipe-process-view";
import { RowVerbs } from "./mash-schedule";

export type NamedOption = { id: string; name: string };
const SELECT = "min-w-0 rounded border bg-background p-2";

export function WaterView({ title, water, profiles, materials, onChange, onEdit, onAdd, onMove }: {
  title: string; water: WaterDraft; profiles: NamedOption[]; materials: NamedOption[];
  onChange?: (patch: Partial<WaterDraft>) => void; onEdit?: (index: number) => void; onAdd?: () => void; onMove?: (index: number, by: -1 | 1) => void;
}) {
  ...
  return <>
    {E.back("Recipe", title)}
    <Field><FieldLabel>Source profile</FieldLabel><select aria-label="Source profile" className={SELECT} {...bind("sourceProfileId")} onChange={(e) => onChange?.({ sourceProfileId: e.target.value })}>
      <option value="">brewery default</option>{profiles.map((p) => <option key={p.id} value={p.id}>{p.name}</option>)}
    </select></Field>
    <Field><FieldLabel>Target profile</FieldLabel><select aria-label="Target profile" className={SELECT} {...bind("targetProfileId")} onChange={(e) => onChange?.({ targetProfileId: e.target.value })}>
    ...
    {water.additions.map((a, i) => (
      <div key={`${i}-${a.materialId}`}>{E.row(name(materials, a.materialId), `${a.qty} ${a.unit} · ${a.stage}`, <RowVerbs onEdit={onEdit && (() => onEdit(i))} onUp={onMove && i > 0 ? () => onMove(i, -1) : undefined} onDown={onMove && i < water.additions.length - 1 ? () => onMove(i, 1) : undefined} />)}</div>
    ))}
    {E.row("Add addition", "material · amount · stage", onAdd ? <button type="button" onClick={onAdd} data-row-action data-tap className="rounded bg-primary/10 px-3 py-1 text-sm text-primary hover:bg-primary/20">Add</button> : E.act("Add"))}
  </>;
}
...
export const additionReady = (f: WaterAdditionFields) => f.materialId !== "" && Number(f.qty) > 0;
...
export function WaterAdditionView({ fields, materials, onChange, footer }: { fields: WaterAdditionFields; materials: NamedOption[]; onChange?: (patch: Partial<WaterAdditionFields>) => void; footer?: ReactNode }) {
  ...
  return <>
    <Field><FieldLabel>Material</FieldLabel><select aria-label="Material" required className={SELECT} {...bind("materialId")} onChange={(e) => onChange?.({ materialId: e.target.value })}>
    ...
    {E.inline(
      ...
      <Field><FieldLabel>Unit</FieldLabel><select aria-label="Unit" className={SELECT} {...bind("unit")} onChange={(e) => onChange?.({ unit: e.target.value })}>{WATER_ADDITION_UNITS.map((u) => <option key={u} value={u}>{u}</option>)}</select></Field>,
    )}
    <Field><FieldLabel>Stage</FieldLabel><select aria-label="Stage" className={SELECT} {...bind("stage")} onChange={(e) => onChange?.({ stage: e.target.value })}>{WATER_ADDITION_STAGES.map((s) => <option key={s} value={s}>{s}</option>)}</select></Field>
