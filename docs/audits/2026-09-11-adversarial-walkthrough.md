...
Walkthrough code revision: `d6eeed7a5e5b66a827ad6ae19272fc3f45539b34` on `adversarial-completion`.
...
All 62 authoritative review cases were executed or inspected against current source. Within the authorized local boundary, 57/57 runnable cases received targeted test, source, or connected browser execution. Thirty satisfied their complete expected outcome; 27 remain Source only because a connected design branch or capability is absent. The other five retain their required policy/legal/manual/assistive-technology boundary. Totals: **30 Proven, 27 Source only, 3 Gated, 2 Not reviewed, 0 Failure**.
...
- Browser coverage: desktop 1440×900 and phone 375×812, pointer and keyboard, six-role same-record deep links, local/global stock, old-record search, customer short shipment, a sign-out/relogin recovered-draft edit and submit, and the portal quantity-input matrix.
- Code proof revision `d6eeed7a5e5b66a827ad6ae19272fc3f45539b34` includes the final bounded confirmation-stock read, truthful shortage-copy repair, and the post-lock restock guard that prevents shipment from racing through a picked-order adjustment. Earlier proof commits were rebased over the #318 integration; the [durable execution ledger](evidence/2026-09-11-f3-execution.md#historical-to-integrated-commit-map) records patch-identical historical→integrated SHA mappings instead of presenting old SHAs as exact-head proof.
- Exact-code full suite: **203 files / 1,932 tests passed** at `662979b991d41d133a647bf6039e97ae8dd7894a`. At `2a6bdd019f884156d9d782a49c0f989dfa5db1ce`, focused portal/order/RLS tests passed 105/105; the final seven-file pure screen/documentation suite passed 121/121.

## Failures repaired

The walkthrough found and fixed ten shared-owner defects: a 375 px composer overflow; fractional portal quantities accepted below an integer-only cart; confirmation that hid source-location stock; a portal short shipment that hid reason/cancellation; direct movements that accepted inactive SKUs or lower-case sample/festival state; shortage copy that contradicted an unpaid invoice; a whole-unit SQL guard that misclassified dual-membership staff; capped confirmation-stock reads; an unsupported assertion that stock existed at another location; and shipment that could race through a picked-order adjustment while physical restock was pending. Each behavior has focused regression proof in the [execution ledger](evidence/2026-09-11-f3-execution.md#fixes-found-by-the-walkthrough). No demonstrated failure remains open, so the Failure count is zero.
...
Walkthrough commit: `d6eeed7a5e5b66a827ad6ae19272fc3f45539b34` (base `5f8743915a44dd8e63039e99c3162befdd14a40d`; exact scenario evidence cites its observed source revision).
...
Observed outcome and evidence: Historical evidence label: **Review question**. Browser evidence at historical `34af080bd65e147a607aba1e83d06f5361b8faca` (patch-identical integrated `e62325f6a7c94b69d7badebb0fa300ff02c05746`): `f3-review-browser/i06-admin-confirm-1440.json`, `i06-admin-confirm-1440.png`, and `i06-admin-confirm.txt` on order `c285f69b-3895-41f4-9a03-fe9753270926`. Fresh RED/GREEN at exact code revision `924cf294f5084443bfa97d213c3642054d1bb166`: `tests/inventory-read-completeness.test.ts` — “assembles every owned stock row beyond the PostgREST row cap” now also proves an order targeting SKU 1,001 receives source on-hand 508 and ATP 505; `tests/orders-review-view.test.ts` proves local source shortage without inventing other-location stock for a single-location or never-stocked SKU. See [exact I06 execution and boundary](evidence/2026-09-11-f3-execution.md#i06).
Design status: Pass — the complete stated outcome was exercised.
...
Observed outcome and evidence: Historical evidence label: **Review question**. Browser evidence at historical `34af080bd65e147a607aba1e83d06f5361b8faca` (patch-identical integrated `e62325f6a7c94b69d7badebb0fa300ff02c05746`): `f3-review-browser/i06-admin-confirm-1440.json` shows source zero versus brewery ATP ten and the intentional-confirm consequence. Fresh RED/GREEN at exact code revision `924cf294f5084443bfa97d213c3642054d1bb166`: `tests/inventory-read-completeness.test.ts` proves source 508/ATP 505 for ordered SKU 1,001, and `tests/orders-review-view.test.ts` proves the quantified local warning no longer infers another stocked location. The integrated lifecycle regression at `47231aed2678610fb13931424d0cc9174eeec035`, `tests/orders-lifecycle.test.ts` — “confirm warns (but does not block) when overselling”, preserves the intentional choice. See [exact O03 execution and boundary](evidence/2026-09-11-f3-execution.md#o03).
Design status: Pass — the complete stated outcome was exercised.
...
Observed outcome and evidence: Historical evidence label: **Review question**. At exact product revision `d6eeed7a5e5b66a827ad6ae19272fc3f45539b34`, `tests/orders-fulfillment.test.ts` — “adjust after pick sets needs_restock; re-pick clears it” proves shipment is refused before re-pick with no shipment or stock movement, then reuses the same request ID successfully after re-pick; “clears needs_restock and writes an order event; no movement” retains the explicit put-back path. See [exact O06 execution and boundary](evidence/2026-09-11-f3-execution.md#o06).
Design status: Pass — the complete stated outcome was exercised.
...
Observed outcome and evidence: Historical evidence label: **Review question**. [CI run 34592472757](https://github.com/energee/mgr-zero/actions/runs/34592472757) exposed two successful transitions at pre-fix head `bcd324f5174edf4ef5c2dbbe87bc6774d3e0eddb`; a local RED reproduced the same outcome. At exact product revision `d6eeed7a5e5b66a827ad6ae19272fc3f45539b34`, `tests/orders-fulfillment.test.ts` — “serializes distinct workers racing shipment and adjustment so only one transition wins” runs eight races through two authenticated clients. Every race has one winner, one terminal event, and zero or one shipment according to that winner; an adjustment-first state blocks shipment until re-pick. See [exact O07 execution and boundary](evidence/2026-09-11-f3-execution.md#o07).
Design status: Pass — the complete stated outcome was exercised.
...
Observed outcome and evidence: Historical evidence label: **Review question**. Newly observed at exact code revision `924cf294f5084443bfa97d213c3642054d1bb166`: customer `1c57e3fd-a645-48c2-ba8b-440e2bc1e7b9` saved draft `bae43e13-73f3-41e5-bde7-83ccabdb32ec` with quantity 2 and note “Before recovery”, signed out, signed back in, reopened **Continue / edit**, changed quantity to 4 and the note to “Edited after recovery”, reviewed $168.00, and submitted that same order ID. Phone artifacts `f3-astra-browser/p01-edit-draft-saved-375.json`, `p01-edit-relogin-orders-375.json`, `p01-edit-recovered-fields-375.json`, `p01-edit-recovered-review-375.json`, and `p01-edit-submitted-375.json` record the connected path. `p01-edit-submitted-sql.txt` records the same customer/ship-to/source/SKU, final quantity 4, created→updated→submitted events, exact portal request IDs, and exactly one row for the target ID. Final-head reload artifacts `p01-final-head-smoke-375.*` and `p01-final-head-smoke-1440.*` at `a5c2b30e6aa06e88a2aac5e07e0ee17a50cd0006` retain the placed order's quantity, PO, edited note, and total. See [exact P01 execution and boundary](evidence/2026-09-11-f3-execution.md#p01).
Design status: Pass — the complete stated outcome was exercised.
Implementation status / proof link: **Proven** — [durable proof index](evidence/2026-09-11-f3-execution.md#p01).
Impact / priority: High — stock, money, access or recovery correctness.
Smallest recommendation or product decision: Retain the cited regression and connected proof.
Owner / follow-up: No follow-up from this audit.
...
Observed outcome and evidence: Historical evidence label: **Review question**. Newly rerun at exact code revision `924cf294f5084443bfa97d213c3642054d1bb166`: desktop 1440×900 and phone 375×812 artifacts `f3-astra-browser/p05-blank-desktop.json`, `p05-decimal.json`, `p05-negative.json`, `p05-huge.json`, `p05-paste-decimal-375.json`, and `p05-sql-counts.txt` cover blank, typed decimal, negative, huge unsafe integer, and an `insertFromPaste` decimal. Every invalid nonblank value stayed visible, showed “Enter whole quantities of zero or more”, disabled Save/Review, and appended no order; a valid integer paste enabled both actions. Patch-identical integrated regressions at `70c044f38377dfc3d198df7ec73cc19dd881380d`: `tests/commands-portal.test.ts` — “rejects fractional packaged quantities at the command and direct RPC boundaries” and “keeps staff fractional edits for a user who also belongs to the order customer”. The UI message is global rather than row-specific/announced, so the complete accessible-error outcome is not established. See [exact P05 execution and boundary](evidence/2026-09-11-f3-execution.md#p05).
Design status: Partial — executable portions were exercised; the complete connected outcome was not established.
...
| B05 | Documented gap | Pass | P01 | Fresh connected proof saved quantity 2, signed out and back in, resumed the same draft, edited it to 4, reviewed, and submitted the same order ID. |
...
| --- | --- | --- | --- |
| J01 | Partial | A01, O01–O08, P01–P08 | Ordering core and recovered-draft submission work; empty cutover, adjacent remedies, buyer notification, and rendered price-change review remain incomplete. |
...
- At `924cf294f5084443bfa97d213c3642054d1bb166`, the bounded-stock and truthful-shortage RED/GREEN batch passed 14/14. The connected P01 recovery/edit/submit and P05 input-matrix browser proof was captured at this revision.
- At final product revision `a5c2b30e6aa06e88a2aac5e07e0ee17a50cd0006`, the same stock/view batch passed 14/14 after nullable empty-line normalization; `bunx tsc --noEmit` and `bun run lint` passed, with only the pre-existing `_request` warning in the public-menu route.
- CI run 34592472757 and a local RED exposed two successful shipment/adjustment transitions at `bcd324f5174edf4ef5c2dbbe87bc6774d3e0eddb`. At exact product revision `d6eeed7a5e5b66a827ad6ae19272fc3f45539b34`, the post-lock restock guard passed `tests/orders-fulfillment.test.ts` 21/21 and the related nine-file order/provider-boundary batch 139/139 after a fresh isolated reset.
- With the final audit content present, the seven-file pure screen/documentation suite passed 121/121, `bunx tsc --noEmit` passed, and `bun run lint` passed with one pre-existing `_request` unused-parameter warning.
- Browser: 1440×900 and 375×812, pointer, keyboard, six roles, source/global stock, old-record search, short-shipment portal result, recovered portal draft edit/submission, and the invalid quantity matrix; session/server closed. True 200% zoom, print, and assistive technology remain unclaimed.
