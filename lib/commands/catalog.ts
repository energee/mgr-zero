import { z } from "zod";
import { cents, defineCommand, defineQuery, latestOf, unwrap, CommandError, STAFF_ROLES } from "./registry";
...
defineCommand({
  ...
  input: z.object({
    id: z.string().uuid().optional(), name: z.string().trim().min(1), style: z.string().optional(), abv: z.number().max(99.99).optional(), // numeric(4,2)
