...
import { z } from "zod";
import { defineQuery, getCommandDefinition, runCommand, STAFF_ROLES, unwrap } from "./registry";
...
const may = (ctx: { role: string }, name: string) => getCommandDefinition(name)?.roles.includes(ctx.role as never) ?? false;
...
defineQuery({
  ...
  handler: async (ctx): Promise<WorkRow[]> => {
    ...
    const [today, pos, routes] = await Promise.all([
      ...
      may(ctx, "list_purchase_orders") ? runCommand("list_purchase_orders", {}, ctx) as Promise<{ id: string; po_no: number; status: string; expected_on: string | null; vendor_name: string | null }[]> : [],
      may(ctx, "list_routes") ? runCommand("list_routes", {}, ctx) as Promise<{ routes: { id: string; name: string | null; delivery_date: string; departed_at: string | null; stops: unknown[] }[] }> : { routes: [] },
    ]);
    const rows: WorkRow[] = [
      ...today.map((t) => ({ kind: KIND[t.reason], label: t.safeLabel, detail: t.detail, href: t.href, verb: VERB[t.reason][0], tone: VERB[t.reason][1], dueAt: t.dueAt })),
      ...pos.map((p) => ({
      ...
      ...routes.routes.map((r) => ({
        kind: "routes" as const, label: r.name ?? "Route", detail: `${r.stops.length} stop${r.stops.length === 1 ? "" : "s"} · ${r.delivery_date}`,
