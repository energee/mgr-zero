...
defineQuery({
  ...
  handler: async (ctx, i) => {
    ...
    const [ln, events, shipment, atp, sourceOnHand] = await Promise.all([
      ...
      unwrap(ctx.db.from("on_hand").select("sku_id, qty").eq("brewery_id", ctx.breweryId).eq("location_id", order.from_location_id)),
    ]);
