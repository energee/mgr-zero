import { z } from "zod";
import { defineCommand, defineQuery, previewCommand, unwrap } from "./registry";
...
defineQuery({
  ...
  input: z.object({ name: z.literal("record_movement"), input: movementInput, conversationId }), roles: "any",
...
defineCommand({
  ...
  input: z.object({ title: z.string().trim().min(1).max(120).optional() }), roles: "any",
  idempotency: "dedupe",
...
defineCommand({
  ...
  input: z.object({ conversationId, role: z.enum(["user", "assistant"]), content: z.string().trim().min(1).max(4000) }), roles: "any",
  idempotency: "dedupe",
...
defineQuery({
  ...
  input: z.object({ conversationId }), roles: "any",
