...
defineQuery({
  name: "list_vendors_and_contracts", description: "Vendors, alphabetical, with lead time and each one's contracts and their drawdown (committed, received, on order, available)",
  input: z.object({}), roles: [...PURCHASING],
  handler: async (ctx) => {
    const [vendors, contracts, balances, materials] = await Promise.all([
