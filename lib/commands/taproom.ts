...
const bucketOf = (days: number) => days <= 30 ? AGE_BUCKETS[0] : days <= 60 ? AGE_BUCKETS[1] : days <= 90 ? AGE_BUCKETS[2] : AGE_BUCKETS[3];
defineQuery({
  ...
  handler: async (ctx) => {
    ...
    const sum = (rows: { qty: number }[]) => rows.reduce((n, r) => n + r.qty, 0);
    const bySize = (totals ?? []).map((t) => ({
      pool_id: t.pool_id as string, pool_name: pool(t.pool_id as string)?.name ?? "", keg_size: t.keg_size as string,
      out: sum(aged.filter((a) => a.pool_id === t.pool_id && a.keg_size === t.keg_size)), total: Number(t.qty),
    })).sort((a, b) => a.pool_name.localeCompare(b.pool_name) || a.keg_size.localeCompare(b.keg_size));
    const out = sum(bySize.map((r) => ({ qty: r.out }))), total = sum(bySize.map((r) => ({ qty: r.total })));
    const aging = AGE_BUCKETS.map((bucket) => {
      const rows = aged.filter((a) => bucketOf(a.days) === bucket);
      return { bucket, kegs: sum(rows), deposit_cents: rows.reduce((n, a) => n + a.qty * Number(pool(a.pool_id)?.deposit_cents ?? 0), 0) };
    });
    ...
    return {
      ...
      customers: [...byCustomer].map(([customer_id, c]) => ({ customer_id, name: customers?.find((x) => x.id === customer_id)?.name ?? "", ...c }))
