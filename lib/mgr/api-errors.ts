...
export const API_ERRORS: ApiError[] = [
  ...
  { code: "preview_required", status: 400, meaning: "A chat-origin command arrived without a confirmed preview bound to its conversation.",
    remedy: "Call `preview_command`, show its effects for confirmation, then send its `previewToken` with the same input and `conversationId`." },
  { code: "invalid_command_metadata", status: 400, meaning: "A registered operation declared an unsafe or contradictory execution policy. A server fault, not a caller one.",
    remedy: "Report it; the operation registration must be corrected before the server can start." },
  ...
  { code: "conflict", status: 409, meaning: "A `requestId` was reused with a different payload, or a confirmed preview changed before its first commit.",
    remedy: "Reuse a requestId only to retry the identical write. Preview current data again after a stale refusal; generate a new requestId for the newly confirmed proposal." },
