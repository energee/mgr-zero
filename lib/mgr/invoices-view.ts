...
/** `timeZone` is breweries.timezone: the paid day is the brewery's day, not paid_at's UTC prefix (#442). */
export function toInvoiceListRow(inv: InvoiceListRecord, role: StaffRole, connected: boolean, timeZone: string): InvoiceListRow {
  ...
  const paid = state === "paid" && inv.paid_at ? ` · paid ${formatDate(inv.paid_at, timeZone)}` : "";
  return {
