...
type Report = { as_of: string; reason: string | null; rows: { variance_bbl: number | null }[] };
...
export function taproomTodayRows(location: { id: string; name: string }, open: TapInterval[], counts: Count[], report: Report): TaproomTodayRow[] {
  ...
  return [
    ...
    { label: "Variance · 4 weeks", detail: `${location.name} · ${variance === null ? (report.reason ?? "comparison unavailable").replaceAll("_", " ") : `${bbl(variance)} expected minus actual`} · as of ${new Date(report.as_of).toLocaleString()}`, href: `/taproom/variance?location=${location.id}&weeks=4`, verb: "Review" },
  ];
}
