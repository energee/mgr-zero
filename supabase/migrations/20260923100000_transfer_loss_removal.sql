-- Cellar transfer loss is a removal (#428). record_cellar_transfer keeps the
-- volume lost on a transfer only in transfers.loss_bbl, and batch completion
-- counts it as already attributed, so its generic-loss remainder leaves it out;
-- the report read only inventory_movements and volume_adjustments, so that
