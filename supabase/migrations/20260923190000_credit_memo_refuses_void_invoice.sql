-- #418: a credit memo (and so a return, which calls this) is refused against an
-- invoice that is no longer owed: written off locally, or voided or deleted in
-- QuickBooks. Crediting one would credit the customer for money they never
-- owe. Mirrors invoiceIsCreditable in lib/mgr/invoice-state.ts, which hides
-- Return on the invoice page in the same states. Otherwise identical to the
-- baseline definition (00001_baseline.sql).
