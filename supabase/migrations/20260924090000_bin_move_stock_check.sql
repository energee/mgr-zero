...
-- lines. Now every line is checked against the source bin:
--   SKU      the selected lot's rows (lot-less rows when no lot is chosen);
--   material the selected lot's rows, else material_bin_on_hand (a lot-less
--            move is only possible for an untracked material, whose bin total
--            is its lot-less stock; explicit null-lot receipt sources keep the
--            lot-less rows);
--   keg      keg_bin_on_hand for the pool and size.
-- Bodies copy the latest definitions (00001_baseline.sql; no later migration
-- redefines either) with only the stock checks and ledger locks changed.
  ...
  for l in select * from public.stock_transfer_lines where transfer_id = p_transfer loop
    ...
    for src in select (e->>'lot_id')::uuid lot_id, (e->>'qty')::numeric qty from jsonb_array_elements(v_sources) e loop
      ...
      if l.sku_id is not null then
        ...
        if v_available < src.qty then raise exception 'insufficient selected FG source stock'; end if;
        ...
        if src.lot_id is not null or coalesce(v_explicit,false) then
          ...
          select coalesce(sum(qty),0) into v_available from public.material_bin_on_hand where brewery_id = t.brewery_id and material_id = l.material_id and bin_id = l.from_bin_id;
        end if;
        if v_available < src.qty then raise exception 'insufficient selected material source stock'; end if;
      elsif src.lot_id is not null then raise exception 'empty kegs have no lot';
      else
        select coalesce(sum(qty),0) into v_available from public.keg_bin_on_hand where brewery_id = t.brewery_id and pool_id = l.keg_pool_id and keg_size = l.keg_size and bin_id = l.from_bin_id;
        if v_available < src.qty then raise exception 'insufficient selected empty keg source stock'; end if;
      end if;
    end loop;
    if l.sku_id is not null then
...
begin
  ...
  if p_sku is not null then
    ...
    if v_available < p_qty then raise exception 'insufficient selected FG source stock'; end if;
    ...
    lock table public.material_movements in share row exclusive mode;
    if p_material_lot is not null then
      select coalesce(sum(qty),0) into v_available from public.material_movements where brewery_id = p_brewery and material_id = p_material and bin_id = p_from_bin and lot_id = p_material_lot;
    else
      select coalesce(sum(qty),0) into v_available from public.material_bin_on_hand where brewery_id = p_brewery and material_id = p_material and bin_id = p_from_bin;
    end if;
    if v_available < p_qty then raise exception 'insufficient selected material source stock'; end if;
    ...
    lock table public.keg_events in share row exclusive mode;
    select coalesce(sum(qty),0) into v_available from public.keg_bin_on_hand where brewery_id = p_brewery and pool_id = p_keg_pool and keg_size = p_keg_size and bin_id = p_from_bin;
    if v_available < p_qty then raise exception 'insufficient selected empty keg source stock'; end if;
    insert into public.keg_events (brewery_id, pool_id, keg_size, location_id, bin_id, qty, reason, note, created_by)
