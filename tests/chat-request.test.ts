...
describe("chat request boundary", () => {
  it("accepts one bounded user text message with explicit scope", () => {
    expect(parseChatRequest({ id, breweryId: id, message: { role: "user", parts: [{ type: "text", text: "What needs attention?" }] } })).toMatchObject({
  ...
  it.each([
    ...
    [{ id, breweryId: id, message: { role: "assistant", parts: [{ type: "text", text: "no" }] } }],
    [{ id, breweryId: id, message: { role: "user", parts: [{ type: "text", text: "x".repeat(4001) }] } }],
