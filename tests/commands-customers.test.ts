...
import { admin, channelId, makeBrewery, makeStaffCtx, seedCatalog } from "./helpers";
...
describe("customer CRUD", () => {
  it("creates a price list, prices a sku, creates a customer on it, adds a ship-to", async () => {
    const pl = await runCommand("upsert_price_list", { name: "2026 wholesale", channelId: await channelId(b.id, "Wholesale") }, ctx) as { id: string };
