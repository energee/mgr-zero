...
it("keeps draft continuation and exact reorder identity, including short shipments", () => {
  ...
  expect(rows[0]).toMatchObject({ verb: "Continue / edit", href: "/portal?draft=order-0" });
  expect(rows[1]).toMatchObject({ verb: "Reorder", href: "/portal?reorder=order-1", warning: true });
});
