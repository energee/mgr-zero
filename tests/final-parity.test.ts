...
describe("final screen parity", () => {
  ...
  it("returns competing order and standing reservations only for the requested SKU and tenant", async () => {
    ...
    const wh = await seedLocation(b.id), tap = await seedLocation(b.id, { name: "Tap", uses: ["taproom" as const] });
    ...
    const rows = await runCommand("get_shortfalls", { skuId: cat.skuId }, ctx) as { skuId: string; onHand: number; allocated: number; atp: number; reservations: { source: string; ref: string; qty: number; orderId?: string }[] }[];
    expect(rows).toHaveLength(1);
    expect(rows[0]).toMatchObject({ skuId: cat.skuId, allocated: 9, atp: -9 });
    expect(rows[0].reservations.map((r) => r.orderId).filter(Boolean).sort()).toEqual(orders.sort());
    expect(rows[0].reservations.find((r) => r.source === "taproom_standing")).toMatchObject({ ref: tap.id, qty: 4 });
  ...
  it("reaches movement 51 with deterministic equal-time ordering and returns frozen receipt facts", async () => {
    ...
    const first = await runCommand("list_movements", { limit: 50 }, ctx) as import("@/lib/supabase/database").Database["public"]["Tables"]["inventory_movements"]["Row"][];
    const second = await runCommand("list_movements", { limit: 50, offset: 50 }, ctx) as import("@/lib/supabase/database").Database["public"]["Tables"]["inventory_movements"]["Row"][];
  ...
  it("sums and projects more than 1,000 filtered reservation and location rows", async () => {
    ...
    const locations = Array.from({ length: 1001 }, (_, n) => ({ id: crypto.randomUUID(), brewery_id: b.id, name: `Tap ${n}`, uses: ["taproom" as const] }));
