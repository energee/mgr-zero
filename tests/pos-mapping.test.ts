...
describe("Square explicit mapping", () => {
  ...
  it("keeps ignored and unavailable variations visible with their mapping history", async () => {
    ...
    expect(rows).toEqual(expect.arrayContaining([
      expect.objectContaining({ externalVariationId: "GUEST", disposition: "ignored", available: true }),
    ...
    expect(JSON.stringify(await runCommand("get_pos_integration_health", {}, adminCtx))).not.toMatch(/access|refresh/i);
  });
});
