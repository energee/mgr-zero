...
async function publicationFixture() {
  ...
  return { brewery, ctx, location, connectionId: connection.data!.id, channel, keg, formatId: poured.data!.id };
}
...
describe("Square durable catalog publication", () => {
  it("preserves seller fields and unrelated locations while changing one owned variation", () => {
    ...
    const addedLocation = prepareSquareCatalogPublication({ brandId: "brand-1", formatId: "format-1",
      brand: "Hazy IPA", format: "Pint", locationId: "L3", priceCents: 725, present: true,
      ownership: "adopted", externalItemId: "ITEM-1", externalVariationId: "VAR-1" }, current);
  ...
  it("persists the exact body and key before I/O, replays unknown outcomes, and records owned identity", async () => {
    const { brewery, ctx, formatId } = await publicationFixture();
    ...
    await expect(publishSquareCatalogItem(ctx, { posLocationId: "L1", formatId }, requestId, client, "publish_pos_menu"))
    ...
    expect(sql(`select ownership||'|'||external_item_id||'|'||external_variation_id from public.pos_catalog_ownership where format_id='${formatId}'`))
  ...
  it("requires verified adoption and a new snapshot/key after a known version conflict", async () => {
    const { brewery, ctx, connectionId, formatId } = await publicationFixture();
    await expect(publishSquareCatalogItem(ctx, { posLocationId: "L1", formatId,
      adoptItemId: "UNKNOWN", adoptVariationId: "UNKNOWN-VAR" }, crypto.randomUUID(), new SquareClient(config, vi.fn()), "publish_pos_item"))
      .rejects.toThrow("Square adoption must select one observed unowned variation");
    ...
    const client = new SquareClient(config, fetch);
    const adoption = { posLocationId: "L1", formatId, adoptItemId: "SELLER-ITEM", adoptVariationId: "SELLER-VAR" };
  ...
  it("serializes concurrent publication to one frozen provider identity", async () => {
    ...
    const results = await Promise.all([1, 2].map(() => publishSquareCatalogItem(ctx,
      { posLocationId: "L1", formatId }, crypto.randomUUID(), client, "publish_pos_menu")));
  ...
  it.each(["role", "connection", "token"] as const)("denies the publication lease after a stale %s", async (stale) => {
    const { brewery, ctx, connectionId, formatId } = await publicationFixture();
    const start = await beginSquarePublication(ctx, { posLocationId: "L1", formatId }, crypto.randomUUID(), "publish_pos_item");
    ...
    await expect(leaseSquarePublication(ctx, start.attemptId)).rejects.toMatchObject({ status: 403 });
  });
});
