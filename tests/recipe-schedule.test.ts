...
import { saccharificationRest, totalMinutes as totalDuration, type MashStep as Step } from "../lib/mgr/recipe-schedule";
...
describe("saccharificationRest", () => {
  ...
  it("prefers duration over order when two steps are both in range", () => {
    const stepped: Step[] = [
      { name: "Beta", kind: "infusion", tempF: 145, minutes: 20 },
      { name: "Alpha", kind: "infusion", tempF: 158, minutes: 40 },
    ];
  ...
  it("answers null when nothing rests in range", () => {
    ...
    expect(saccharificationRest([{ name: "Mash-out", kind: "direct heat", tempF: 168, minutes: 10 }]))
      .toBeNull();
  });
});
