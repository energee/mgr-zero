  ...
  const matrix: MatrixCase[] = [
    ...
    },
    {
      ...
      input: async role => {
        ...
        const { data: route } = await admin.from("routes").insert({ brewery_id: brewery.id, delivery_date: "2026-09-08", name: "matrix", driver_user_id: contexts()[role].userId }).select("id").single();
