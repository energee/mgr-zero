...
describe("customer writes are locked once an order leaves draft", () => {
  it("a direct update on a confirmed order affects 0 rows", async () => {
    const { data: confirmed } = await admin.from("orders")
      .update({ status: "confirmed" }).eq("id", order.id).select().single();
    expect(confirmed!.status).toBe("confirmed");
    const db = await asUser(custUser.email);
    const { data, error } = await db.from("orders").update({ note: "x" }).eq("id", order.id).select();
    expect(error).toBeNull();
    expect(data!.length).toBe(0);
  });

  it("a direct update on a submitted order's note affects 0 rows", async () => {
    ...
    const db = await asUser(custUser.email);
    const { data, error } = await db.from("orders").update({ note: "x" }).eq("id", o!.id).select();
    expect(error).toBeNull();
    expect(data!.length).toBe(0);
  });

  it("a direct write to a submitted order's lines affects 0 rows", async () => {
    ...
    const db = await asUser(custUser.email);
    const { data, error } = await db.from("order_lines").update({ qty_ordered: 99 }).eq("id", line!.id).select();
    expect(error).toBeNull();
    expect(data!.length).toBe(0);
  });
});
